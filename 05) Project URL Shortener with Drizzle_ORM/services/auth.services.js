...
export const getUserByEmail = async (email) =>{
    const [user] = await db
    .select().from(usersTable).where(eq(usersTable.email, email));
...
export const createUser = async ({name, email, password}) =>{
    return db
    .insert(usersTable).values({name, email, password}).$returningId();
...
export const authenticateUser = async({req, res, name, user, email}) =>{
...
export const insertVerifyEmailToken = async ({userId, token}) => {
  ...
  try {
    await tx
    .delete(verifyEmailTokensTable).where(lt(verifyEmailTokensTable.expiresAt, sql`CURRENT_TIMESTAMP`));

    await tx
    .delete(verifyEmailTokensTable).where(eq(verifyEmailTokensTable.userId, userId));
...
// here we add URL Api's version in this code.....
export const createVerifyEmailLink = async ({email, token}) =>{
...
// here we use inner join method of MySQL and update the code......
...
export const sendNewVerifyEmailLink = async ({userId, email}) =>{
  const randomToken = await generateRandomToken();
    ...
    // here we used mjml version in the code.............
    ...
    const htmlOutput = (await mjml2html(filledTemplate)).html;
...
export const updateUserByName = async ({userId, name, avatarUrl}) =>{
  ...
  // Only include avatarUrl in update if it's provided
...
export async function createUserWithOauth({
  ...
  const user = await db.transaction(async(trx) => {
    // Generate a random password for OAuth users since they won't use password login
